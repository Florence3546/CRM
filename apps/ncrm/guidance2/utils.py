...
class TimeSpliter(object):
    _cycle = (('day', '天'), ('week', '周'), ('ps_week', '周（派生）'), ('month', '月'))
    ...
    def week_spliter(self, start_time, end_time, end_date = 6):
        week_cycle = 7
        days = (end_time - start_time).days
        week_days = start_time.weekday()
        week_rest_days = end_date - week_days
        week_rest_days = week_cycle + week_rest_days if week_rest_days < 0 else week_rest_days
    ...
    def ps_week_spliter(self, start_time, end_time):
        return self.week_spliter(start_time, end_time, end_date = 4)
