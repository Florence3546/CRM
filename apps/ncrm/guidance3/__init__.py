...
class StatisticsHelper(object):
    ...
    def __init__(self, xfgroups, indicators, start_time, end_time):
    ...
    def __call__(self, staff_data):
        ...
        for xfgroups in self.xfgroups:
            user_data = staff_data.get(xfgroups, {})
    ...
    def to_dict(self):
        return {
            'name': ','.join(xfgroups.name for xfgroups in self.xfgroups),
...
class PerformanceStatistics(object):
    ...
    def _load_all_entity(self, xfgroups, indicator, time_scope, is_force = False):
        loader = Loader([xfgroups], *time_scope)
        staff_mapping = loader.loading([indicator], is_force)

        all_entity = []
        indicatore_data = staff_mapping.get(xfgroups)
    ...
    def performance_statistics_shopids(self, xfgroups, indicator_name, time_scope):
        indicator = IndicatorManager.get_indicator(indicator_name)
        all_entity = self._load_all_entity(xfgroups, indicator, time_scope)
    ...
    def performance_statistics_sumval(self, xfgroups, indicator_name, time_scope):
        indicator = IndicatorManager.get_indicator(indicator_name)
        all_entity = self._load_all_entity(xfgroups, indicator, time_scope, True)
        sumval, _, _ = indicator.sum_func(all_entity)
        return sumval

    def rander_table_data(self, time_scope, cycle, indicators, xfgroups, is_force = False):
        all_result, time_scope_list, indicators = self(time_scope, cycle, indicators, xfgroups, is_force)
        ...
        statistic_data = []
        for xfgroup, row_data in all_result.iteritems():
            result = {'name_cn':xfgroup.name, 'xfgroup_id':xfgroup.id, 'metric_list':[]}
    ...
    def __call__(self, time_scope, cycle, indicators, xfgroups, is_force = False):
        xfgroups = list(xfgroups)
        ...
        try:
            ...
            for xfgroups in xfgroups:
                statis_data = []
                if len(time_scope_list) > 1:
                    result = StatisticsHelper([xfgroups], indicators, *time_scope)(staff_mapping)
                    statis_data.append(result)

                for start_time, end_time in time_scope_list:
                    result = StatisticsHelper([xfgroups], indicators, start_time, end_time)(staff_mapping)
                    statis_data.append(result)

                all_result[xfgroups] = statis_data

            if len(time_scope_list) > 1:
                time_scope_list.insert(0, time_scope)
            return all_result, time_scope_list, indicators
        except Exception, e:
            log.exception('ncrm_guidance3 error, %s' % e)
            return {}, [], []
