define(["template", 'widget/ajax/ajax', 'widget/alert/alert', 'moment'], function(template, ajax, alert, moment) {
    "use strict";
    ...
    var show = function(options) {
        ...
        obj.on('hidden.bs.modal', function() {
            if (reset) {
                $('#' + options.keywordId).trigger('show.rob', [options]);
            }
            $(this).remove();
            destroyChar();
            // ws.close();
        ...
        ajax.ajax('manual_rob_rank', {
            ...
            if (data.limitError != '') {
                ...
                if (data.limitError == 'version_limit') {
                    title = 'Sorry!当前版本不支持';
                    body = '<div class="f16">亲，您订购的版本不支持手动抢排名哦，请升级到双引擎版！</div><div class="tc mt10"><button type="button" class="btn btn-primary" data-dismiss="modal">知道了</button><a class="btn btn-primary ml20" href="/web/upgrade_suggest/" target="_blank">立即升级</a></div></div>';
                }
    ...
    var showRobMsg = function(options, msg, ws) {
        ...
        if (msg.result_flag == 'ok' || msg.result_flag == 'nearly_ok') {
            ...
            if (options.target != "auto_rob_rank") {
                options.method=='manual'&&trObj.trigger('update.rob_rank', ['success', Number(msg.price).toFixed(2), msg.platform]);
            }
        ...
        if (msg.result_flag == 'done' || msg.result_flag == 'failed') {
            ...
            if (options.target != "auto_rob_rank") {
                options.method=='manual'&&trObj.trigger('update.rob_rank', ['fail']);
            }
            flag = 'finish';
            ws.close();
        }

        getAndSetChat(options, msg);
    ...
    var getAndSetChat = function(options, msg) {
        if (!char) {
