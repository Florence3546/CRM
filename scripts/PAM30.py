...
class PAMIE:
    ...
    def __init__(self, url = None, timeOut = 3000, Visible = 1):
    ...
    def _frameWait(self, frame = None):
        ...
        try:
            ...
            while readyCount < self.busyTuner and timeLeft > 0:
                if timeLeft % 500 == 0:
                    pass#print '_wait():readyCount=%s < self.busyTuner=%s and timeLeft=%s >0 ' % (readyCount, self.busyTuner, timeLeft)
                try:
    ...
    def _wait(self):
        ...
        try:
            while readyCount < self.busyTuner and timeLeft > 0:
                if timeLeft % 500 == 0:
                    #print '_wait():readyCount=%s < self.busyTuner=%s and timeLeft=%s >0 ' % (readyCount, self.busyTuner, timeLeft)
                    pass
                try:
                    doc = self._ie.Document
                except:# if the document never gets itself together this will timeout
                    time.sleep(0.1)
                    timeLeft -= 10
                    continue
...
##        try:
    ...
    def findWindow(self, title, indexNum = 1):
        ...
        found = False
        while not found:
            shellWnd = win32com.client.dynamic.Dispatch('Shell.Application')
        ...
        if self.showDebugging: print ("** findWindow() did not find the " + title + "-" + str(indexNum) + " window.")
        return None
