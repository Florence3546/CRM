...
//                    }
                    ...
                    this.ranges[range] = options.ranges[range][0];
                }
			...
			if (typeof options.call_back == 'function'){
				this.call_back=options.call_back;
			}
...
//                this.hide();
//            } 
				 if(!(this.element.find('.date_value').text()==label)&&this.call_back){
					this.element.find('.date_value').text(label);
					form.find('input[name="last_day"]').val(this.ranges[label]); //将改变的值赋给隐藏域
					this.call_back(this.ranges[label],form);
				 }
				 this.hide();
        },
