...
$.fn.dataTableExt.afnSortData['custom-text']=function(oSettings,iColumn){return $.map(oSettings.oApi._fnGetTrNodes(oSettings),function(tr,i){var td=oSettings.oApi._fnGetTdNodes(oSettings,i)[iColumn];if(tr.className.indexOf('unsort')!=-1){return 'unsort'}else{return $(td).find("span:first").hasClass('custom')?Number($(td).find('span:first').text()):$(td).text();}})};$.fn.dataTableExt.afnSortData['dom-input']=function(oSettings,iColumn){return $.map(oSettings.oApi._fnGetTrNodes(oSettings),function(tr,i){return $('td:eq('+iColumn+') input',tr).val();});};
jQuery.fn.dataTableExt.oSort['custom-asc']=function(x,y){if(x=="unsort")return-1;if(y=="unsort")return 1;x=parseFloat(x);y=parseFloat(y);return((x<y)?-1:((x>y)?1:0))};jQuery.fn.dataTableExt.oSort['custom-desc']=function(x,y){if(x=="unsort")return-1;if(y=="unsort")return 1;x=parseFloat(x);y=parseFloat(y);return((x<y)?1:((x>y)?-1:0))};$.fn.dataTableExt.afnSortData['dom-text']=function(oSettings,iColumn){return $.map(oSettings.oApi._fnGetTrNodes(oSettings),function(tr,i){return $('td:eq('+iColumn+') span:eq(0)',tr).text()})};
