...
//    }
    ...
    var init_dom = function () {
        ...
        // 点击打开选择创意图片框
        $('#item_cart').off('click', '[name="creative_img"]');
        $('#item_cart').on('click', '[name="creative_img"]', function () {
        ...
        $('#fancybox_add_item .btn_OK').click(function () {
	        if ($('#item_cart .none_crt_img').length) {
		        PT.alert('有宝贝未选择创意图片，请先选择图片');
		        return;
	        } else if ($('#item_cart .loading_tag:visible').length) {
		        PT.alert('正在加载数据，请等待加载完后再操作');
		        return;
		    } else if ($('#item_cart [name="title_length_prompt"].overflow').length>0) {
		        PT.alert('有些创意标题的长度超过20个汉字');
		        return;
	        } else if ($('#item_cart [name="title_length_prompt"].blank').length>0) {
                PT.alert('有些创意标题为空');
                return;
            }
//            check_danger_cats();
...
//        },
        ...
        getorcreate_adg_title_list_callback: function (adg_title_list, item_id) {
            var adg_title_obj = $('#item_cart tr[item_id="'+item_id+'"] .adg_title');
