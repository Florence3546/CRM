...
PT.HealthCheck = function () {
	...
	var init_loading_layout = function(){
		PT.sendDajax({'function':'web_get_health_layout','obj_id':$("#adgroup_id").val(),'check_type':'adgroup','flag':0});
	}
	...
	var send_data=function(){
		PT.sendDajax({'function':'web_get_health_layout','obj_id':$("#adgroup_id").val(),'check_type':'adgroup','flag':1});
	}
    ...
    return {
		...
		show_flag:['icon-trophy','icon-globe','icon-hand-up','icon-truck','icon-gbp'],		
        init:function () {
		...
		init_layout:function(adgroup_id,data_list){
			...
			setTimeout(function(){PT.sendDajax({'function':'web_get_health_items','cache_key':'adgroup_'+adgroup_id,'client_position':-1});},PT.HealthCheck.sent_time_interval);
		},
		...
		set_show_result:function(base_dict,result_dict){
			...
			client_position += 1;
			var length = result_dict['data'].length;
