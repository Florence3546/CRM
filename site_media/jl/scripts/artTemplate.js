...
//add by zhongjinfeng   格式化参数
template.helper('$format',function(numerator , denominator, multiple, point){
	return (numerator * multiple / denominator).toFixed(point);
});

template.helper('$filter_match',function(numerator){
	var scorp_list={1:'精',2:'中',4:'广'};
	return scorp_list[numerator];
});

template.helper('$page_dict',function(index){
	var page_dict={'1-5':'第1-5名',
				   '6-12':'第6-12名',
				   '13-17':'第1页底部',
				   '18-29':'第2页右侧',
				   '30-34':'第2页底部',
	...
	return page_dict[index];
});

template.helper('$replace',function(str,b,d){
	return str.replace(b,d);
});

template.helper('$date_format',function(format_str,dt){
	var currnt_dt=dt==undefined?new Date():new Date(Date.parse(dt.substr(0,dt.indexOf('.'))));
	return currnt_dt.format(format_str);
});
...
template.helper('$comp',function(b,s){
	return Number(b)>Number(s)?2:Number(b)<Number(s)?1:0;
});