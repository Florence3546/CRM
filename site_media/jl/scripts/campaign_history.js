...
PT.Camp_history = function () {
	...
	var init_table=function(){
		history_table=$('#camp_history_table').dataTable({
	...
	var init_dom=function () {
		...
		$('#camp_search_btn').click(function(){
			search_camp();
		});
	...
	var search_camp=function() {
		var search_word=$('#title_input').val();
	...
	var send_date=function (sSource, aoData, fnCallback, oSettings){
		...
		PT.Camp_history.page_info=page_info;
		PT.Camp_history.callBack=fnCallback;
		PT.sendDajax({
    ...
    return {
		...
		append_table_data:function(page_info_server,data){
			...
			PT.Camp_history.callBack(page_info_server);//填充数据
			table.show();
