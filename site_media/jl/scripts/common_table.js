...
PT.Table.BaseRowObj.prototype.forecast_order=function(tip){//tip 用来判断是否弹出消息框
    ...
    if(this.forecast_order_btn.length>0){
	    if(!this.forecast_order_limit(_tip)){
            this.forecast_order_btn.html("<img src='/site_media/jl/img/forecast_orde_ajax.gif'>");
...
//设置预估出价
//PT.Table.BaseRowObj.prototype.set_forecast_order=function(type,json){
//	var template_str="";
//	if(type==1){
//		template_str=template.render('template_forecast_select_fail');
//		this.forecast_order_btn.replaceWith(template_str);
//	}else if(type==2){
//		template_str=template.render('template_forecast_select',json);
//		this.forecast_order_btn.replaceWith(template_str);
//		this.nRow.data('forecast_data',json.result);
//		this.chose_current_rank();
//		
//		var that=this;
//		$('#forecast_order_'+this.kw_id).change(function(){
//			var forecast_data=that.nRow.data('forecast_data');
//				var index=$(this).val();
//				for (var i=0;i<forecast_data.length;i++){
//					if(forecast_data[i].page==index){
//						that.new_price_input.val(forecast_data[i].price);
//						that.update_style();
...
PT.Table.BaseRowObj.prototype.set_want_order=function(){
	...
	var want_order=	PT.instance_table.want_order, limit_price = Number(PT.instance_table.want_order_limit);
	var opt = select_obj.find('option:eq('+want_order+')');
	if (opt.length==0) {
		opt = select_obj.find('option:last');
	}
	var max_price = Number(opt.val().split('-')[0]);
	var new_price = limit_price?Math.min(max_price, limit_price):max_price;
...
//根据当前出价选中预估排名
PT.Table.BaseRowObj.prototype.match_current_rank=function(){
	var select_obj = this.v_forecast_select();
	if (select_obj.length==0) return;
...
PT.Table.BaseTableObj=function(table_id,temp_id){
	...
	//默认隐藏列
	this.DEFAULT_HIDE_COLUMN=['create_days','cpm','avgpos','favcount','favctr','favpay','g_pv','g_ctr','g_competition'];
	//隐藏列的值保存到每行的data中
...
PT.Table.BaseTableObj.prototype.display_word_parts=function(data){
    $('#id_set_filter_layer').html(template.render('template_ban_word',$.evalJSON(data)));
    $.fancybox.open([{href:'#id_set_filter_layer'}], {helpers:{
        title : {type : 'outside'},
...
PT.Table.BaseTableObj.prototype.init_ajax_event=function(){
	...
	this.table_obj.find('.icon-ban-circle').click(function(){
	   that.row_cache[this.parentNode.parentNode.parentNode.id].parse_keyword();
	});
...
PT.Table.BaseTableObj.prototype.start_bulk_forecast_order=function(){
	var msg = '预估排名执行时间较长，您确定要执行吗？<div class="r_color mart_6">说明：因为淘宝的千人千面算法，且关键词没有实际竞价，预估结果可能不准</div>';
...
PT.Table.BaseTableObj.prototype.bulk_forecast_order = function () {
	var that=this, count = 0, kw_id_list = [];
	var checked_rows = that.table_obj.find('tbody input:checked').closest('tr');
	checked_rows.each(function () {
		if (count<PT.Table.TB_LIMIT_FPRECAST_COUNT) {
			var qscore = Number($(this).find('td.qscore').html());
			if (qscore<=PT.Table.TB_LIMIT_QSCORE) {
				forecast_order_btn.replaceWith(PT.Table.TB_LIMIT_FORECAST_ORFER);
				return true;
			}
			var forecast_order_btn = $(this).find('.forecast_order_btn');
	        if (forecast_order_btn.length>0) {
		        forecast_order_btn.html("<img src='/site_media/jl/img/forecast_orde_ajax.gif'>");
	            kw_id_list.push($(this).attr('id'));
	            count++;
	        }
		} else {
			return false;
	...
	PT.sendDajax({'function':'web_get_keywords_rankingforecast','kw_id_list':kw_id_list});
}

//批量预估回调
PT.Table.BaseTableObj.prototype.get_keywords_rankingforecast_callback = function (forecast_data) {
	//处理单个关键词的排名数据
    var handle_rank_data = function (rank_data) {
        var result = [], page_data = [], page_data_rt = [], page_data_rb = [], page_data_b = [], page_pos = 17, position = 0, page_no, price, max_price, min_price;
        //分页处理
        for (var i=0;i<Math.ceil(rank_data.length/page_pos);i++) {
            ...
            for (var j in page_data_rt) {
                ...
                position++;
                result.push(['第'+position+'名：'+price+'元', price]);
            }
            //右侧后八位(第3页以后为12)
            if (page_data_rb.length>0) {
                if (page_data_rb.length==1) {
                    ...
                    result.push(['第'+(position+1)+'~'+(position+page_data_rb.length)+'名：'+max_price+'~'+min_price+'元', max_price+'-'+min_price]);
                    position += page_data_rb.length;
                }
            }
            //底部五位
            if (page_data_b.length>0) {
                if (page_data_b.length==1) {
                    ...
                    position += page_data_b.length;
                }
            }
        }
        result.push([position+'名以后：'+(min_price-0.01).toFixed(2)+'元', (min_price-0.01).toFixed(2)]);
        return result;
    };
    
    // 主逻辑
    var that = this, sum =0, failed = 0, msg = '';
    for (var kw_id in forecast_data) {
	    sum++;
	    var rank_data = forecast_data[kw_id];
		var row_obj = that.row_cache[kw_id];
	    if ($.isArray(rank_data) && rank_data.length>0) {
//		    row_obj.nRow.data('forecast_data', rank_data);
		    var template_str = template.render('template_forecast_select', {'kw_id':kw_id, 'result':handle_rank_data(rank_data)});
		    row_obj.forecast_order_btn.replaceWith(template_str);
		    row_obj.v_forecast_select().change(function () {
			    var r_obj = that.row_cache[$(this).closest('tr').attr('id')];
			    r_obj.new_price_input.val(this.value.split('-')[0]);
			    r_obj.update_style();
			    r_obj.calc_action_count();
		    });
	    } else {
		    rank_data = rank_data.length>0?rank_data:'淘宝返回预估排名数据为空';
		    var template_str = template.render('template_forecast_fail', {'error_msg':rank_data});
		    row_obj.nRow.find('.forecast_order_btn').replaceWith(template_str); // 为避免缓存干扰，不要使用row_obj.forecast_order_btn
		    row_obj.nRow.find('select.forecast_fail').change(function () {
			    if (this.value=='1') {
				    var r_obj = that.row_cache[$(this).closest('tr').attr('id')];
				    $(this).replaceWith('<a href="javascript:;"class="forecast_order_btn"><img src="/site_media/jl/img/forecast_orde_ajax.gif"></a>');
				    r_obj.forecast_order();
			    }
		    });
		    failed++;
		    continue;
	    ...
	    // 根据批量修改里的估价设置来调整新出价
	    if (this.want_order=='') {
		    row_obj.match_current_rank();
	    } else {
		    row_obj.set_want_order();
	    }
    }
    if (sum>1) {
	    if (failed==0) {
		    ...
		    msg = '批量预估完成，部分关键词预估失败，请重试或联系客服';
	    } else {
		    msg = '批量预估失败，请重试或联系客服';
	    }
	    PT.alert(msg);
//    } else if (sum==1 && failed==1) {
//	    PT.alert('预估排名失败，请重试或联系客服');
    } else if (sum==0) {
	    PT.alert('预估排名完成');
    }
    this.want_order='';
