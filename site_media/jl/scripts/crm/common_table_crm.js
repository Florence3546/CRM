...
PT.Table.BaseRowObj.prototype.is_del=function(){
	if(this.label_code!=undefined&&this.label_code.indexOf('SL')!=-1){
		this.kw_del=1;
	}
}
...
PT.Table.BaseRowObj.prototype.chose_current_rank=function(){
	...
	if(new_price>=forecast_data[0].price){
		...
		for (var i=1;i<forecast_data.length-1;i++){
			var prev=forecast_data[i-1].price;
			var current=forecast_data[i].price;
...
PT.Table.BaseTableObj=function(table_id,temp_id){
	...
	//用来隐藏列对应的字典
	this.COLUM_DICT={'qscore':5,'create_days':6,'impressions':7,'click':8,'ctr':9,'cost':10,'ppc':11,'cpm':12,'avgpos':13,'favcount':14,'paycount':15,'pay':16,'conv':17,'roi':18,'g_pv':19,'g_click':20,'g_ctr':21,'g_cpc':22,'g_competition':23};
	//默认隐藏列
	this.DEFAULT_HIDE_COLUMN=['cpm','favcount'];
	//隐藏列的值保存到每行的data中
...
PT.Table.BaseTableObj.prototype.init_event=function(){
	...
	$('#id_curwords_submit').click(function(){
		var msg='',row=that.row_list();
		var limit_count=0,limit_val=$('#user_limit_price input').val();
		that.row_list().each(function(){
			var obj=that.row_cache[this.id];
			if (obj.is_modified()&&obj.v_new_price()>that.KW_LIMIT_PRICE){
				limit_count++;
			}
		});
		
		if(limit_count>0){
			msg='<span class="r_color">您有'+limit_count+'个关键词超过了'+limit_val+'元</span></br>';
			//PT.confirm('您要提交的关键词中有'+limit_count+'个关键词超过了'+limit_val+'元,您要继续提交吗？',that.next_submit_verify,[],that);
		}
		
	if((that.plus_count+that.fall_count+that.match_count+that.del_count)>0){
		msg+='即将提交：加价:'+that.plus_count+' 降价:'+that.fall_count+' 匹配:'+that.match_count+' 删除:'+that.del_count
...
/*	$('#smart_optimize_btn').click(function(){
	...
	$('.go_setup3').click(function(){
		var obj=$('#batch_optimize_box');
		obj.removeClass('setup2').addClass('setup3');
	});
	
	$('.go_setup1').click(function(){
		var obj=$('#batch_optimize_box');
		obj.removeClass('setup2').addClass('setup1');
	});	
...
//			PT.alert('限价必须是00.5到99.99的数字，请重新填写');
//			$(this).focus();
//			return;
...
//		$(this).parent().parent().data('obj').change_type($(this));
//	});
...
//	});
	...
	$('#show_rob_box').click(function(){
		if(that.show_rob_box()){ //返回false表示没有符合条件的关键词
			...
			$('#id_zone').attr('disabled',true); //禁止下拉菜单改变再次触发查询
...
PT.Table.BaseTableObj.prototype.curwords_submit=function(){
	...
	this.row_list().each(function(){
		var obj=that.row_cache[this.id];
		var obj=that.row_cache[this.id];
		var shop_id=obj.nRow.attr('shop_id');
		if (obj.is_modified()){
			var kw_dict={
			...
			if (shop_dict.hasOwnProperty(shop_id)) {
				shop_dict[shop_id].push(kw_dict);
			} else {
				shop_dict[shop_id]=[kw_dict];
			}
			adg_id_list.push(parseInt(obj.adgroup_id));
		}
	});

	shop_dict=$.toJSON(shop_dict);
	adg_id_list = $.toJSON(adg_id_list);
	var is_jumped=parseInt($('#base_is_jumped').val());
	PT.show_loading('正在提交数据');
	PT.sendDajax({'function': 'crm_curwords_submit','shop_dict': shop_dict,'adg_id_list':adg_id_list,'is_jumped':is_jumped});
}

//自定义csv数据格式
PT.Table.BaseTableObj.prototype.creat_scv_data=function(){
	var data_str='关键字\t质量得分\t养词天数\t展现量\t点击量\t点击率\t总花费\t平均点击花费\t千次点击花费\t昨日平均排名\t收藏量\t成交量\t成交额\t转化率\t投资回报\t全网展现指数\t全网点击指数\t全网点击率\t全网市场均价\t全网竞争度\n';
...
//提交后的回调函数
PT.Table.BaseTableObj.prototype.curwords_submit_call_back=function(updated_list, deleted_list, top_deleted_list){
	
	for (var i=0;i<deleted_list.length;i++){
		//$('#'+json.del_kw[i]).data('obj').delete_row();
		this.row_cache[deleted_list[i]].delete_row();
	}
	
	for (var i=0;i<top_deleted_list.length;i++){
		//$('#'+json.del_kw[i]).data('obj').delete_row();
		this.row_cache[top_deleted_list[i]].delete_row();
	}
	
	for (var i=0;i<updated_list.length;i++){
		//$('#'+json.update_kw[i]).data('obj').update_row();
		this.row_cache[updated_list[i]].update_row();
	}
	/*
...
PT.Table.BaseTableObj.prototype.bulk_plus=function(base_type,delta,mode,limit){
	...
	if(mode=='int'&&delta!=undefined&&isNaN(delta)){
		PT.alert('请输入有效数字，必须是0.05至99.99之间的数字');
		return;		
	}
	...
	if(mode=='int'&&(delta<0.05 || delta>99.99)){
		PT.alert('请输入有效数字，必须是0.05至99.99之间的数字');
		return;			
	}
...
PT.Table.BaseTableObj.prototype.start_rob_ranking=function(keyword_list){
	...
	PT.sendDajax({'function':'web_rob_ranking','keyword_list':$.toJSON(keyword_list),'lock_status':1,'ip':$('#ip_zone').val()});
}
