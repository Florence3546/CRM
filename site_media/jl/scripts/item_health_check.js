...
PT.HealthCheck = function () {
	...
	var init_item_report = function(){
		$("#link_item_report").on(
			"click",function(){
				$("#health_check").hide();
				$("#item_analysis").hide();
				//PT.sendDajax({'function':'web_get_item_report','item_id':68131155});
	...
	var init_item_analysis = function(){
		$("#link_item_analysis").on(
			"click",function(){
				$("#health_check").hide();
				$("#item_report").hide();
	...
	var excute_check = function(){
		...
		$("#excute_health_check").on(
			"click",function(){
				...
				$("#loading").show();
				PT.sendDajax({'function':'web_get_health_check','obj_id':$("#adgroup_id").val(),'check_type':'adgroup','flag':0});
			}
		)
	}
	...
	var init_recheck = function(){
		$("#re_health_check").on(
			"click",function(){
				...
				$("#progrss_top").html('');			
				$("#health_check").css("padding-top","0");
				PT.sendDajax({'function':'web_get_health_check','obj_id':$("#adgroup_id").val(),'check_type':'adgroup','flag':1})
				$("#show_check_result").hide();
				$("#loading").show();
			}
		)		
    ...
    return {
		...
		set_check_result:function(data_dict){
			...
			if (data_dict.dangerous_items.length > 0){
				...
				//$("#dangerous_items_div").html(template.render('dangerous_table_template',{"items":data_dict.safe_items[0].items}));							
				init_table();
			} else {
