 ...
 * @since			2014.05.22
 * @author			钟进峰
 * @function        自定义表单验证 $('##').vaildata({call_back:function});
...
!function ($) {
  ...
  $.fn.vaildata = function (options) {
    ...
    var range = function (obj,rule) {
      ...
      if (!(/^[0-9]*[0-9][0-9]*$/).test($.trim(obj.value))||value<min||value>max) {
    ...
    this.submit(function (e) {
      var inputs, selects,pass = 1,that=this;
      e.preventDefault();

      options=$.extend({},{placement:'top'},options)
      inputs = $('input[data-rule]', this);
