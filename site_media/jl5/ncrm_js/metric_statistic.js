...
PT.MetricStatistic = function() {
        ...
        var limit_refresh_cycle = function(){
            ...
            if ( end_date.getDate() >= 1 && end_date.getHours() >= 18){
        ...
        var init_dom = function() {
            ...
            require(['dom', 'gallery/datetimepicker/1.1/index'], function(DOM, Datetimepicker) {
                ...
                $('.data-calendar').on('click', 'td[data-date]', function () {
                    setTimeout(function () {
                        ...
                        if (start_time && end_time && start_time<=end_time) {
                            ...
                            PT.sendDajax({
                                'function': 'ncrm_get_metric_list',
                                'start_time': start_time,
                                'end_time': end_time,
                                'callback': 'PT.MetricStatistic.get_metric_list_callback'
                            });
                        }
                    }, 100);
                });
            ...
            $('#search_form').on('click', '.checkbox_all', function () {
                $(this).parent('label').nextAll(':visible').find('input').attr('checked', $(this)[0].checked);
            });

            $('#search_form').on('change', 'input[type=checkbox]', function () {
            ...
            $('#refresh_data').click(function () {
                if ( check_time() && check_limit_date() ){
                    PT.confirm("亲，请注意，<b style='color:red;'>每月1号18点前，可刷新上月数据，1号18点后就只能刷新本月数据</b>，您是否要进行 <b style='color:red;'>绩效校准 </b>？", limit_refresh_cycle);
                }
            });
            ...
            $('a.search_shops').click(function () {
                var xfgroup_id = $(this).parent('td').siblings('td[xfgroup_id]').attr('xfgroup_id'),
                ...
                window.open('/ncrm/myworkbench/?metric_statistic_args='+[xfgroup_id, metric_name, start_date, end_date].join(','));
            });
            ...
            $('a.export_data').click(function () {
                ...
                PT.sendDajax({
                    'function': 'ncrm_get_export_metric_data',
                    'xfgroup_id': xfgroup_id,
            ...
            $('a.refresh_data').click(function () {
                ...
                PT.show_loading('正在加载');
                PT.sendDajax({
                    'function': 'ncrm_refresh_metric_data',
                    'xfgroup_id': xfgroup_id,
                    'metric_name': metric_name,
                    'data_no': data_no,
                    'start_date': start_date,
            ...
            $('a.metric_chart').click(function () {
                var td_obj = $(this).parent('td'),
                      data_objs = [],
                      my_name = td_obj.attr('xfgroup_name'),
                      metric_name = td_obj.attr('metric_name'),
                      metric_unit = td_obj.attr('unit');
                var xfgroup_name_list = $.map($('#table_event_statistic>tbody td.xfgroup_name'), function (elem) {return $(elem).text()});
                ...
                $.each(xfgroup_name_list, function (i, xfgroup_name) {
                    ...
                    series_cfg_list.push({
                        ...
                        'visible': xfgroup_name==my_name?true:false,
                        'unit': metric_unit,
                        'opposite': false,
                        'is_axis': i===0?1:0,
                        'yAxis': 0
    ...
    return {
        ...
        refresh_metric_data_callback: function (xfgroup_id, metric_name, data_no, result) {
            $('#table_event_statistic td[xfgroup_id='+xfgroup_id+'][metric_name='+metric_name+']').siblings('[data_no='+data_no+']').children('a.search_shops').html(result);
        },
        export_metric_data_callback: function(xfgroup_id, metric_name, data_no, event_list){
            var data_str = '',
                xfgroup_name = $.trim($('input[type=checkbox][name=server_id][value='+xfgroup_id+']').parent().text()),
                ...
                file_name = xfgroup_name + '_' + metric_str + '_' + th_name + '.csv';
        ...
        get_metric_list_callback: function (error, metric_list) {
            if (error) {
                PT.alert(error);
            } else {
                $('#metric_list label:gt(0)').remove();
                $.each(metric_list, function (i, metric) {
                    $('#metric_list').append(
                ...
                $('#metric_list :checkbox').attr('checked', true);
            }
        }
    };
}();
