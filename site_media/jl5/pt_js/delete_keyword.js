...
tempClass.prototype = PT.Table.BaseTableObj.prototype;
PT.Delete_keyword.history_table.prototype = new tempClass();
...
PT.Delete_keyword.history_table.prototype.call_history_back=function(json){
    ...
    for (i in json){
        ...
        json[i]['loop']=i;
        dom+=template.compile(pt_tpm['delete_keyword.tpm.html'])(json[i]);
        count++;
    }
...
PT.Delete_keyword.history_table.prototype.init_history_event=function(){
    ...
    this.table_obj.on('click','.match',function(){
        that.row_cache[$(this).parents('tr:first').attr('id')].change_match_scorp();
    });

    //改变出价时调用
    this.table_obj.on('change', '.new_price', function(){
        that.row_cache[$(this).parents('tr:first').attr('id')].history_change_new_price();
    });
    ...
    //改变选中关键词的匹配模式
    $('#history_bulk_match_btn').on('change',function(e,value){
      that.history_bulk_match(value);
    });

    //设置关键词出价
    $('#history_set_new_price').on('change',function(e,value){
        that.history_set_new_price(value);
    });
    ...
    $('#history_recover_kw').click(function(){
        ...
        row_objs.each(function(){
            var obj=that.row_cache[$(this).parents('tr:first').attr('id')];
...
PT.Delete_keyword.history_table.prototype.sort_history_table=function(){
    ...
    this.data_table=this.table_obj.dataTable({
        ...
        "aoColumns":[
            {"bSortable": true,"sSortDataType": "dom-text", "sType": "numeric","sClass": "no_back_img tc"},
            {"bSortable": false},
            {"bSortable": true},
            {"bSortable": true},
            {"bSortable": true},
    ...
    $(window).on('scroll.PT.Table',function(){
        ...
        if (body_offset >= obj_offset) {
            ...
            if (this.fixed_header == undefined) {
                this.fixed_header = new FixedHeader(that.data_table, {
...
//搜索并排序
PT.Delete_keyword.history_table.prototype.search_word_and_sort=function(search_word){
    var that=this;
    this.row_list().each(function(){
        var obj=that.row_cache[this.id];
