...
PT.Duplicate_check = function () {
	...
	var init_dupl_list=function (){
		PT.dupl_list_table=$('#dupl_kw_list').dataTable({
			...
			"aoColumns": [
				...
				{"bSortable": true,"sClass":'rel'}
			],
			"fnServerData": send_date,//指定函数发起请求
			"iDisplayLength": DUPLICATE_CHECK_SIZE,
			"sDom":"<'row-fluid't<'p10 h20 mb20'<'span12'i><'span12 tr'p mr10>>",
			"oLanguage": {
				"sZeroRecords": "没有记录",
				"sEmptyTable":"没有数据",
				"sInfoEmpty": "显示0条记录",
	...
	//将模板转化为dom对象
	var get_dom_4template=function (secore,json){
		var tr_arr=[];
		for (d in json){
			if(!isNaN(d)){
				tr_arr.push($(template.compile(secore)(json[d])));
			}
		}
		return tr_arr;
	};
	...
	var init_dom=function (){
		...
		$('#dupl_kw_list tbody').on('change','input',function(){
			if(!($(this).attr('checked')=='checked') && ($('#dupl_kw_list thead tr input').attr("checked")=="checked")){
				$('#dupl_kw_list thead tr input').attr("checked",false);
			}
		});
		...
		$('#comm_submit_btn').click(function(){
			...
			switch(del_type){
				case 2:
					...
					if(check_total>0){
						PT.confirm('您确认按条件批量删除包含以下'+check_total+'个关键词的重复词吗？', condition_del, [kw_arr]);
					}else{
	...
	var init_ajax_dom_first=function(){
		...
		$('#dupl_kw_list tbody td .pointer').on('click', function () {
		    var parent_box = $(this).parents("td").prev().find("input");
            NOW_TR = $(this).parents('tr')[0];
			if($(this).find('.row-details').hasClass("open")){
				$(this).find('.row-details').removeClass('open').html('&#xe624;');
			}else{
				$(this).find('.row-details').addClass('open').html('&#xe623;');
			}
            ...
            //当上一个td中的checkbox选中时，动态加载的dom中的checkbox也需要选中
            if($(parent_box).attr("checked")=="checked"){
                $(parent_box).parent().parent().next().find('table').find('input').each(function(index){
                    if($(this).attr("disabled")!="disabled"){
                         $(this).attr('checked',true);
                    }
                });
            }else{
                $(parent_box).parent().parent().next().children().find('input').attr('checked',false)
            }
        });
	};
	...
	var init_ajax_dom_second=function(){
		...
		// 复选框事件
        $('.kid_box').change(function(){
            if($(this).attr("checked")=="checked"){
                $(this).parent().parent().next().find('table').find('input').each(function(index){
                    if($(this).attr("disabled")!="disabled"){
                         $(this).attr('checked',true);
                    }
                });
        ...
        $('.sub_box').change(function(){
            if(!($(this).attr("checked")=="checked")){
                $(this).parent().parent().parent().parent().find("thead tr th").find(".top_box").attr("checked",false)
            }
        });
    ...
    return {
		...
		dupl_list: function (page_info_server,data,garbage_words_len){
		    ...
		    if(duplicate_len==0){
			    ...
			    var template_data,datatable_arr;
		            ...
		            template_data=get_dom_4template(pt_tpm['duplicate_check.tpm.html'],data);
		...
		dupl_detail: function(data){
			...
			var tr_str='';
			for (var i=0;i<data.length;i++){
				tr_str += template.compile(pt_tpm['duplicate_check_detail_tr.tpm.html'])(data[i]);
			}
			var dupl_detail_str= template.compile(pt_tpm['duplicate_check_detail.tpm.html'])({'tr_str':tr_str});
		...
		delete_result: function (del_type, del_count, failed_count, del_kw_list, is_refresh) {
			...
			if(del_count===0){
				if (failed_count>0) {
					PT.alert('亲，淘宝接口错误，删词失败，请稍后重试');
				} else{
					PT.alert('亲，您的店铺中不存在符合条件的重复词');
				}
			} else if(failed_count>0 | is_refresh | del_type!='manual') {
				var msg = '删除成功'+del_count+'个,';
				if (failed_count>0) {
					msg += '删除失败'+failed_count+'个';
				}
				msg += '5秒后重新排查';
				PT.alert(msg);
				setTimeout('PT.dupl_list_table.fnDraw();', 1000*5);
			}
		}
    };
}();
