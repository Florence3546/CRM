...
PT.Rob_rank=function(){
	...
	var init_dom = function() {
		...
		$('.rob_rank_help').on('click', function(){
			var alert_str = '<p>1.抢排名不是锁排名，抢排名只在您手工设置时，卡一次位置，而不能固定排名位置，锁排名是淘宝禁止的功能。</p>\
	<p>2.为了防止淘宝API调用超限，导致其他用户正常的关键词改价无法使用，已经抢到排名的关键词，不能再次抢排名。</p>\
	<p>3.下表中不显示质量得分6分以下的关键词，质量得分太低，出再高的价都可能达不到排名，且可能导致淘宝API调用超限。</p>';
			$.alert({
				backdrop:'static',
				title:'抢排名说明',
				body: alert_str});
...
//显示抢排名信息
PT.Table.robRow.prototype.show_rob_info=function(mode,new_price,rank,tips){
	var msg='';
	this.nRow.find('.rob_set_box').addClass('dn');
	...
	if(mode>0){
		...
		switch (mode) {
			case 1:
				...
				this.nRow.find('td:first').removeClass('poi');
				break;
			case 2:
				msg='限价内只能抢到第'+ rank +'名，' + tips;
				this.nRow.find('.rob_set').addClass('fail');
				break;
		}
	}else{
		msg = '抢排名失败，' + tips;
		if(tips == '出价到99元也抢不到排名') {
			this.nRow.find('.kid_box').attr({'disabled':true,'checked':false}).addClass('disabled');
		}
	}
...
PT.Table.robRow.prototype.check_forecast_data=function(){
	var forecast_data=this.nRow.data('forecast_data');
	if(forecast_data === undefined && !this.prompt_price.hasClass('disabled')){
		this.prompt_price.html("<img src='/site_media/jl/img/forecast_orde_ajax.gif'>").fadeIn();
...
PT.Table.robRow.prototype.forecast_data_back=function(result,data){
	if(result){
		...
		this.prompt_price.html("获取失败");
		this.prompt_price.addClass('disabled');
	}
};

//设置建议限价
PT.Table.robRow.prototype.set_prompt_price=function(){
	if (this.prompt_price.hasClass('disabled')) {
		return;
	}
...
PT.Table.robRow.prototype.get_price_by_rank=function(rank){
	...
	if (forecast_data.length > rank) {
		forecast_price = forecast_data[rank-1];
	} else {
		forecast_price = forecast_data[forecast_data.length-1];
	}
	return parseFloat(forecast_price/100);
};
...
PT.Table.robRow.prototype.is_rob_rank=function(){
	var forecast_data=this.nRow.data('forecast_data');
	if(forecast_data && forecast_data[forecast_data.length-1]>9900){
		this.show_rob_info(0, 0, 0, '出价到99元也抢不到排名');
		return false;
	}
	return true;
};
...
PT.Table.robTable.prototype.layout_keyword=function(data){
	...
	for (var i=0;i<kw_count;i++){
		tr_str+=template.compile(pt_tpm['rob_rank.tpm.html'])(data[i]);
	}
...
PT.Table.robTable.prototype.init_event=function(){
	...
	this.table_obj.find('.prompt_price').click(function(){
		...
		var tr_id=$(this).closest('tr').attr('id'),
			this_row = that.row_cache[tr_id],
			rob_min=this_row.o_min.val(),
			rob_max=this_row.o_max.val();
		if(rob_min && rob_max){
			this_row.set_prompt_price();
		}else{
			PT.light_msg('提醒',"请先填写期望排名");
		}
	});

	this.table_obj.find('.rob_min,.rob_max').focus(function(){
		var tr_id=$(this).closest('tr').attr('id');
	...
	this.table_obj.find('.rob_min,.rob_max,.rob_limit').blur(function(){
		...
		if($(this).hasClass('rob_limit')){
			...
			// }
			if(rob_min && rob_max) {
				that.row_cache[tr_id].set_prompt_price();
			}
		}
	});
...
PT.Table.robTable.prototype.bulk_rob_reset=function(){
	...
	if(this.has_checked_row()){
		this.checked_row().each(function(){
			...
			jq_tr.find('.rob_set_box').removeClass('dn');
...
PT.Table.robTable.prototype.bulk_rob_ranking=function(){
	var that=this;
	if(this.has_checked_row()){
		var that=this,error_count=0,unable_count=0,unfull_count=0,total_count=0,keyword_list={},msg,min_obj,max_obj,limit_obj,rob_min,rob_max,rob_limit;
		...
		this.checked_row().each(function(){
			...
			if(!obj.is_rob_rank()){
				unable_count ++;
			}else if(rob_min!==''||rob_max!==''||rob_limit!==''){
				if(that.check_rob_input(min_obj,[1,100],true)&&that.check_rob_input(max_obj,[1,100],true)&&that.check_rob_input(limit_obj,[0.05,PT.Table.KW_LIMIT_PRICE],false)){
					if(rob_min>rob_max){ //确保最小值小于最大值
						min_obj.addClass('orange');
						max_obj.addClass('orange');
						error_count++;
					}else{
						min_obj.removeClass('orange');
						max_obj.removeClass('orange');
				...
				min_obj.removeClass('orange');
				max_obj.removeClass('orange');
				limit_obj.removeClass('orange');
				unfull_count++;
			}

		});
		if(total_count>(error_count+unfull_count+unable_count)){
			msg='当前选中'+total_count+'个';
			if (error_count) {
				msg += ',<span class="red">输入有误'+error_count+'个</span>';
			}
			if (unfull_count) {
				msg += ',<span class="red">未填写'+unfull_count+'个</span>';
			}
			msg += '。您确定要批量抢排名吗？';
...
PT.Table.robTable.prototype.check_input=function(obj,limit_arr,is_int){
	...
	if(!obj.val()){
		...
		obj.addClass('orange').val('');
		PT.light_msg('提醒','请输入数字');
		return false;
	}
	if (num<limit_arr[0]) {
	...
	obj.removeClass('orange').val(input_val);
	return true;
};
...
PT.Table.robTable.prototype.rob_ranking_call_back=function(data){
	...
	setTimeout(function(){
		msg="";
		if (rob_info_change) {
			msg += "达到期望排名"+rob_info_change+"个";
		}
		if (rob_info_unchange) {
			msg += " 未达期望排名"+rob_info_unchange+"个";
		}
		if (rob_error) {
			msg += " 失败"+rob_error+"个";
		}
		PT.light_msg('抢排名结果', msg);
	},16);
