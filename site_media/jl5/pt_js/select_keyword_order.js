...
PT.SelectKeywordOrder = function() {
    ...
    var word_filter,
        ...
        get_data = function(type) {
            ...
            switch (select_type) {
                case 'quick':
                    send_data.mode = $('input[name="radio_type"]:checked').attr('id') == 'traffic'? 'traffic': 'precise';
                    break;
                case 'precise':
        ...
        creat_csv_data = function() {
            ...
            $.map(oSettings.aiDisplay, function(i) {
                var nTr = oSettings.aoData[i].nTr;
                if (nTr.children[0].children[0].checked) {
                    data_str += nTr.children[1].children[0].innerText.replace(/\n/g,'')  + '\t' + Number(nTr.children[2].innerText) + '\t' + Number(nTr.children[3].innerText) + '\t' + Number(nTr.children[4].innerText) + '\t' + Number(nTr.children[5].innerText) + '\t' + Number(nTr.children[6].innerText)+'\n';
                }
            });
            return data_str;
        },
