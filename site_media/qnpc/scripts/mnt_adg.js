...
PT.MntAdg = function () {
	...
	var init_table=function(){
		mnt_adg_table=$('#adgroup_table').dataTable({
			...
			"iDisplayLength": 20,
			"sDom":"<'row-fluid't<'row-fluid marb_24'<'span6'i><'span6 sui-pagination pagination-large'p>>",
			"aoColumns": [
				{"bSortable": false},
...
//		});
		...
		$('body').on('click', '.mnt_oper.single', function(){
			...
			}else if(camp_mnt_type==2 || camp_mnt_type==4){
				set_limit_price(this);
			}else{
				confirm_mnt_oper(mnt_oper,adg_id);
			}
...
//			}
	...
	return {
        ...
        update_mnt_back: function(adg_id_list,status){
			...
			if(camp_mnt_type==1 || camp_mnt_type == 3) {
