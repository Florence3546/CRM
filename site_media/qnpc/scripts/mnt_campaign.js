...
PT.MntCampaign = function () {
	...
	if(mnt_type==1&& mnt_type==3){
		var camp_budget_min=100, camp_budget_max=300;
	}else{
	...
	var init_dom=function(){
		...
		$('#id_budget').attr('data-original-title','建议设在'+camp_budget_min+'~'+camp_budget_max+'元之间');
...
//			}});
          $.confirm({
            title:'确定解除计划吗？',
            body:'<p>1、系统会停止托管您的宝贝，但不会改变当前计划的推广状态<br>2、您可以重新开启，但需要重新选择计划和宝贝等等</p>',
            okHide:function(){
...
//				update_quick_oper_tip();
              ...
              $.alert({
                title:oper_name + '成功',
                body:'正在执行智能改价,24小时后您可以再次对计划进行调整！'
              });
			}else if(result=='0'){
		...
		update_camp_back:function(status){
			...
			if(status){
				...
				$('#camp_mnt_oper').attr('mnt_oper',1-status).text(mnt_oper_str).fadeOut();
			}else{
			...
			update_quick_oper_tip();
			// $('#camp_mnt_oper').attr('mnt_oper',1-status).text(mnt_oper_str).toggleClass('btn-success').toggleClass('btn-danger');
